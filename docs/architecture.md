...
### ADR-011: Menu Bar Control (Supersedes ADR-009 Transform Mode)

**Status:** Accepted (2025-12-21)

**Context:** ADR-009's Transform Mode evolved through multiple iterations (ADR-010 vertical layout), becoming increasingly complex:
- Custom UI components (~800 lines)
- BroadcastChannel IPC for state sync
- Window transform/restore logic
- Same-screen detection
- Device selection dropdowns in floating UI

After reviewing similar apps (Hopp, AltTab), a simpler pattern emerged: **native menu bar control**.

**Decision:** Replace Transform Mode with **Menu Bar Control** - a native macOS tray icon with simple menu during screen sharing:

```
Tray Icon + Menu (when sharing):
┌──────────────────────┐
...
6. **~80 lines of Rust** vs ~800 lines previous
    ...
    TrayIconBuilder::new()
        ...
        .on_menu_event(|app, event| {
            match event.id.as_ref() {
                ...
                "stop" => app.emit("tray://stop-sharing", ()),
                "leave" => app.emit("tray://leave-meeting", ()),
                _ => {}
            }
        })
        .build(app)?;
...
| Aspect | ADR-003 (Float) | ADR-009 (Transform) | ADR-011 (Menu Bar) |
|--------|-----------------|---------------------|-------------------|
| Windows | 2 | 1 (transforms) | 1 (unchanged) |
| Custom UI | Complex | Complex | None |
| Lines of code | ~500 | ~800 | ~80 |
| IPC | BroadcastChannel | Events | Tauri events |
| State sync | Complex | Moderate | None |
| Native feel | Custom | Custom | 100% native |
...
**Consequences:**
- Remove SharerControlBar, SharerControlBarVertical components
- Remove FloatingControlBarPage
- Remove floatingBarChannel IPC
- Remove transform mode commands
- Add tray.rs module (~80 lines)
- Add tray icon assets (normal + sharing state)

**Platform Support:**
| Platform | Support | Notes |
|----------|---------|-------|
| macOS | Full | Native NSStatusItem |
| Windows | Works | System tray (different location) |
| Linux | Varies | AppIndicator/StatusNotifier |

**References:**
- AltTab app menu bar design
- Hopp tray implementation
- Tauri System Tray documentation
...
_Updated: 2025-12-21 (ADR-011 added - Menu Bar replaces Transform Mode)_
_For: BMad_
